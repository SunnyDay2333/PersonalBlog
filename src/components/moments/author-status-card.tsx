...
// 暗色：暗色半透明底 + 微光边框（与 MomentCard 同源 bg-card）
// compact 模式：移动端水平单行
// ============================================================
...
export function AuthorStatusCard({ momentCount, compact }: AuthorStatusCardProps) {
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-3 rounded-xl border border-border/30 bg-white/55 px-4 py-3 dark:border-white/[0.08] dark:bg-card/45 lg:hidden">
        <img
          src={AVATAR_URL}
          alt={DISPLAY_NAME}
  ...
  return (
    <div className="group flex flex-col items-center rounded-2xl border border-border/30 bg-white/55 px-3 py-5 text-center transition-shadow duration-300 hover:shadow-[0_2px_16px_rgba(0,0,0,0.04)] hover:border-border/40 dark:border-white/[0.08] dark:bg-card/45">
