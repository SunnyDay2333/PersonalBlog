...
export function AuthorStatusCard({ momentCount, compact }: AuthorStatusCardProps) {
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-3 rounded-xl border border-border/30 bg-white/55 px-4 py-3 lg:hidden">
        <img
          src={AVATAR_URL}
          alt={DISPLAY_NAME}
          className="h-9 w-9 rounded-full border-2 border-border/30 object-cover"
        />
